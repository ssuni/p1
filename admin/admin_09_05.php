...
while ( $aSelQuery = @mysql_fetch_array($rSelQuery) ){
	if ( fnGetHostName($aSelQuery["statistics_refer"]) == "" ){
		$arrHost["북마크"]++;
	}else{
		$arrHost[fnGetHostName($aSelQuery["statistics_refer"])]++;
	}
}
@arsort($arrHost);
			...
			<table width="99%" border="0" cellspacing="0" cellpadding="0">
				...
				<tr>
					...
					<!-- 타이틀 -->
						<table width="99%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td class="main_title_01">경로별 접속 현황 </td>
							</tr>
						</table>
