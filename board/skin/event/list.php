...
<div id="boardSkin">
	<div class="subjectBox"><?=$Data["subject"]?> &nbsp;&nbsp; 
		<? if (strtotime($Data["entStart"]." 00:00:00") <= time() && time() <= strtotime($Data["entEnd"]." 23:59:59")) { ?>
			<img src="/board/skin/<?=$boardSet["skin"];?>/images/btn_ing.gif" alt="이벤트 진행중" class="middle" />
		<? } else if (date("Y-m-d") > $Data["entStart"]) { ?>
			<img src="/board/skin/<?=$boardSet["skin"];?>/images/btn_end.gif" alt="이벤트 종료" class="middle" />
		<? } ?>
	</div>
	<div class="infor"><ul><li><strong>이벤트 설명</strong> : <?=$Data["entSubject"];?></li></ul></div>
	<div class="infor"><ul><li><strong>이벤트 기간</strong> : <?=$Data["entStart"];?> ~ <?=$Data["entEnd"];?> </li></ul></div>
	<!--<div class="infor">
		<ul>
...
<div id="boardSkin">
	...
	<table summary="" id="boardList">
		...
		<?		for( $i = 0; $i < $lTmp; $i++ ){
			...
			<tr class="notice">
				<td class="td1">공지</td>
				<td class="td4"><img src="<?=($Data[$i]["savefile"][0])?$Data[$i]["savefile"][0]:'/board/skin/'.$boardSet["skin"].'/images/noimg.gif';?>" width="218" height="110" /></td>
			<?if(count($List["categoryarr"]) > 0){?>
				<td class="td2">&nbsp;</td>
			<?}?>
				<td class="td3">
				...
				<td class="td5"><?=$Data[$i]["viewlink"]?><img src="/board/skin/<?=$boardSet["skin"];?>/images/btn_view.gif"/></a></td>
			</tr>
		<?		}
				## 공지일경우 ##
				else{	?>
		...
		<?		}
			...
			<!--글이 없을 때-->
			<tr>
				<td colspan="5" align="center" height="100">등록된 게시글이 없습니다.</td>
			</tr>
		<?	}	?>
		</tbody>
	</table>
