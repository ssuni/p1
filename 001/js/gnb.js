...
(function( $ ) {
	...
	$.fn.menuInit = function() {
		...
		function config(){
			...
			var b = mobileChk();
			if(b){
				$this.find(".gnb_list > ul > li").click(oneOver);						
				$this.find(".gnb_list > ul > li > a").attr("href","javascript:void(0);");
			}else{
				$this.find(".gnb_list > ul > li").mouseover(oneOver);						
			}
			...
			$this.find(".gnb_list .sub a").each(function (i) {				
		...
		function onePlayFn(n,b){
			...
			$this.find(".gnb_list > ul > li").each(function (i) {				
				if(i==n){
