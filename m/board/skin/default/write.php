<link href="/board/skin/default/css/board.css" rel="stylesheet" type="text/css" />
...
<?	}	?>
	<table width="100%" id="writeForm">
		...
		<tbody>
			<tr>
				<th>제목</th>
				<td><input name="strSubject" type="text" id="strSubject" style="width:95%;" class="textForm" value="<?=$Data["subject"]?>" itemname="제목" required><span class="tt">
		...
		<?	if( $_SESS["ss_level"] > 2 ){	?>
			<tr>
				<th>비밀번호</th>
				<td><input name="strPass" type="password" id="strPass" class="textForm" style="width:40%;" itemname="비밀번호" required></td>
			</tr>
		<?	}	?>
			<tr>
				<th>연락처</th>
			 ...
			 - 
			 <input name="strMobile2" type="text" id="strMobile2" style="width:100px;" maxlength="4" class="textForm" value="<?=$Data["mobile"][1]?>" <?=( $_SESSION["ss_level"] > 2 ) ? "itemname='휴대폰' required numeric" : "" ?>>
			 - 
			 <input name="strMobile3" type="text" id="strMobile3" style="width:100px;" maxlength="4" class="textForm" value="<?=$Data["mobile"][2]?>" <?=( $_SESSION["ss_level"] > 2 ) ? "itemname='휴대폰' required numeric" : "" ?>></td>
			</tr>
	...
	<?	for( $i = 0; $i < $boardSet["addfilenumber"]; $i++ ){	?>
			<tr>
				...
				<input type="file" name="strSaveFile[]" id="strSaveFile" style="width:70%;height:36px;" class="textForm"><?if(($tb=='banner' || $tb=='banner2') && $i==0) echo "(적정사이즈 : 285 X 144)"; else if(($tb=='banner' || $tb=='banner2') && $i==1) echo "(적정사이즈 : 252 X 155)";?></td>
			</tr>
	<?	}	?>
	<?	if( $_SESSION["ss_level"] <= 2 ){	?>
		...
		<?	}	?>
			<tr>
				<th>자동등록</th>
				<td><input name="wr_key" type="text" id="wr_key" class="textForm" style="width:40%;" itemname="자동등록방지" required>&nbsp;<?=$norobot_str?> <font color="#FF6600">붉은색 글씨</font>를 차례로 입력해주세요.</td>
			</tr>
			<tr>
				<td colspan="2">
	...
	<div class="btnArea3">
	<img src="/board/skin/default/images/btn_ok.gif" style="cursor:pointer" onclick="submitContents(document.frmMain);" class="middle">&nbsp;
	<img src="/board/skin/default/images/btn_cancel.gif" align="top" onClick="javascript:location.href='<?=$PHP_SELF?>?tb=<?=$tb?>&sField=<?=$sField?>&sSearch=<?=$sSearch?>&sField2=<?=$sField2?>&sKeyword=<?=$sKeyword?>&sGP=<?=$sGP?>&sSecret=<?=$sSecret?>&act=list';" style="cursor:pointer;" align="absmiddle"></div>
</div>
</form>
