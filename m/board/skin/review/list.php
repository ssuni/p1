	...
	<table summary="" id="boardList">
		...
		<?		for( $i = 0; $i < $lTmp; $i++ ){
			...
			<tr>
				...
				<td class="td2" <?=($Data[$i]['number']==$_GET['tNum'])?'style="background-color:#f5f5f5"':'';?>><img src="<?if($Data[$i]["savefile"][0]!=''){?><?=$Data[$i]["savefile"][0]?><?}else{?>/board/skin/<?=$boardSet["skin"];?>/images/noimg.jpg<?}?>" class="noimg" width="150" ></a></td>
