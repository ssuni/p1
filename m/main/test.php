...
$cn = 0;
?>
<? include "../include/head_test.php" ?>	
<? include "../include/head_proc.php" ?>	
<? for ( $i = 0 ; $i < $nPopCnt; $i++ ) {    // 팝업창 추가?>
	<div id="popup<?=$arrPopup[$i]['idx']?>" style="cursor:pointer; position:absolute; z-index:<?=(10 + $i )?>; width:<?=$arrPopup[$i]['width']?>px; height:<?=$arrPopup[$i]['height']?>; left:<?=$arrPopup[$i]['left']?>px; top: <?=$arrPopup[$i]['top']?>px; display:block;">
