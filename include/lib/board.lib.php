    ...
    ob_start();
		echo '<ul class="category">';
			if ($category) {
				...
				echo '<li class="'.$select_all.'bg"><a href="'.$_SERVER['PHP_SELF'].'?tb='.$_GET['tb'].'&sField=">전체보기</a></li>';
				for($i=0; $i<count($category); $i++) {
					if (trim($category[$i])) {
						$select = ($_GET['sField']==($i+1))?'select ':'';
						echo '<li class="'.$select.'bg"><a href="'.$_SERVER['PHP_SELF'].'?tb='.$_GET['tb'].'&sField='.($i+1).'">'.$category[$i].'</a></li>';
					}
				}
			}
...
function getListSkinFile($skin) {
	...
	if ($_SESSION["ss_level"] == 1) {
		if(file_exists($_SERVER['DOCUMENT_ROOT']."/board/skin/".$skin."/list_admin.php"))
		{
			$list_file = $_SERVER['DOCUMENT_ROOT']."/board/skin/".$skin."/list_admin.php";
		}
		else
		{
			$list_file = $_SERVER['DOCUMENT_ROOT']."/board/skin/".$skin."/list.php";
		}
	} else {
		$list_file = $_SERVER['DOCUMENT_ROOT']."/board/skin/".$skin."/list.php";
	}
